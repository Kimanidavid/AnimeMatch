...
import { Search, Play, Heart, Plus, Loader, Filter, X, ExternalLink, Monitor } from 'lucide-react';
...
export function BingePage() {
  ...
  useEffect(() => {
    applyFilters();
  }, [animeList, filterGenre, filterType, sortBy, searchQuery]);

  const loadTopAnime = async () => {
    setLoading(true);
    const data = await animeApi.getTopAnime(100);
    setAnimeList(data);
    setLoading(false);
  };
  ...
  const handleSearch = async (query: string) => {
    ...
    const results = await animeApi.searchAnime(query, 50);
  ...
  const applyFilters = () => {
    ...
    if (filterGenre) {
      filtered = filtered.filter(anime =>
        anime.genres?.some(g => g.name === filterGenre)
      );
    }

    if (filterType) {
      filtered = filtered.filter(anime => anime.type === filterType);
    }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="comic-panel comic-panel-primary max-w-2xl w-full my-8">
        <div className="p-8">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="comic-button flex-1 px-3 py-2 bg-[#4A7C7E] text-white font-bold disabled:opacity-50"
                >
                  {inWatchlist ? 'IN WATCHLIST' : 'ADD TO WATCHLIST'}
                </button>
                <button
                  onClick={onClose}
                  className="comic-button px-3 py-2 bg-[#A63F4F] text-white font-bold"
                >
                  CLOSE
                </button>
              </div>
          ...
          {anime.synopsis && (
            <div className="comic-panel bg-white p-4 mb-6 border-2 border-black">
              <h3 className="font-bold text-black text-lg mb-2">SYNOPSIS:</h3>
              <p className="font-semibold text-black">{anime.synopsis}</p>
            </div>
          )}

          <div className="comic-panel-teal comic-panel p-6">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <a
                ...
                className="comic-button px-4 py-3 bg-[#F7931E] text-black font-bold flex items-center justify-center gap-2 hover:shadow-lg"
              >
                <Play size={16} />
                CRUNCHYROLL
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="comic-button px-4 py-3 bg-[#A63F4F] text-white font-bold flex items-center justify-center gap-2 hover:shadow-lg"
              >
                <Play size={16} />
                NETFLIX
                <ExternalLink size={14} />
